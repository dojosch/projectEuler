const largestProductinaSeries = (series) => {
    ...
    for(let i = 0; i < (thousandDigits.length - series); i++) {
        let tmpArray = thousandDigits.slice(i, i + series)
        if(!tmpArray.some(x => x = 0)) {
            let tmpProduct = 1;
            for(let j = 0; j < series; j++) {
                tmpProduct *= tmpArray[j]
            }
            largestProduct = largestProduct > tmpProduct ? largestProduct : tmpProduct
        }
    }
